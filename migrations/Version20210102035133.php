 ...
 */
final class Version20210102035133 extends AbstractMigration
{
    ...
    public function up(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__cart AS SELECT id, buyer_id, product_id, added_date, quantity FROM cart');
        $this->addSql('DROP TABLE cart');
        $this->addSql('CREATE TABLE cart (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, buyer_id INTEGER DEFAULT NULL, product_id INTEGER DEFAULT NULL, added_date DATETIME NOT NULL, quantity INTEGER NOT NULL, total_price DOUBLE PRECISION NOT NULL, CONSTRAINT FK_BA388B76C755722 FOREIGN KEY (buyer_id) REFERENCES "user" (id) NOT DEFERRABLE INITIALLY IMMEDIATE, CONSTRAINT FK_BA388B74584665A FOREIGN KEY (product_id) REFERENCES product (id) NOT DEFERRABLE INITIALLY IMMEDIATE)');
        $this->addSql('INSERT INTO cart (id, buyer_id, product_id, added_date, quantity) SELECT id, buyer_id, product_id, added_date, quantity FROM __temp__cart');
    ...
    public function down(Schema $schema) : void
    {
        ...
        $this->addSql('CREATE TEMPORARY TABLE __temp__cart AS SELECT id, buyer_id, product_id, added_date, quantity FROM cart');
        $this->addSql('DROP TABLE cart');
        $this->addSql('CREATE TABLE cart (id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, buyer_id INTEGER DEFAULT NULL, product_id INTEGER DEFAULT NULL, added_date DATETIME NOT NULL, quantity INTEGER NOT NULL)');
        $this->addSql('INSERT INTO cart (id, buyer_id, product_id, added_date, quantity) SELECT id, buyer_id, product_id, added_date, quantity FROM __temp__cart');
        $this->addSql('DROP TABLE __temp__cart');
        $this->addSql('CREATE INDEX IDX_BA388B76C755722 ON cart (buyer_id)');
        $this->addSql('CREATE INDEX IDX_BA388B74584665A ON cart (product_id)');
